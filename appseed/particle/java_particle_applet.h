...
class _Frame extends Frame
{
   ...
   public _Frame(_Applet parent)
   {
      ...
      particle = new _Tetris(10,20,24,this);
      add("Center",particle);
   ...
   public bool action(Event evt,Object arg)
   {
      ...
      if(evt.target == pause)
      {
         particle.atPause = true;
      }
      if(evt.target == smallerCell)
      {
         if(particle.cellSizeInPixels != 18)
         {
            particle.init(particle.widthInCells,particle.heightInCells,18);
            particle.resize(particle.preferredSize());
            pack();
         }
      }
      if(evt.target == standardCell)
      {
         if(particle.cellSizeInPixels != 24)
         {
            particle.init(particle.widthInCells,particle.heightInCells,24);
            particle.resize(particle.preferredSize());
            pack();
         }
      }
      ...
      {
         if(particle.cellSizeInPixels != 30)
         {
            particle.init(particle.widthInCells,particle.heightInCells,30);
            particle.resize(particle.preferredSize());
            pack();
         }
      }
      ...
      {
         if((particle.widthInCells != 10) || (particle.heightInCells != 20))
         {
            particle.init(10,20,particle.cellSizeInPixels);
            particle.resize(particle.preferredSize());
            pack();
         }
      }
      ...
      {
         if((particle.widthInCells != 10) || (particle.heightInCells != 12))
         {
            particle.init(10,12,particle.cellSizeInPixels);
            particle.resize(particle.preferredSize());
            pack();
         }
      }
      if(evt.target == wider)
      {
         if((particle.widthInCells != 20) || (particle.heightInCells != 20))
         {
            particle.init(20,20,particle.cellSizeInPixels);
            particle.resize(particle.preferredSize());
            pack();
         }
      }
      ...
      {
         if((particle.widthInCells != 10) || (particle.heightInCells != 30))
         {
            particle.init(10,30,particle.cellSizeInPixels);
            particle.resize(particle.preferredSize());
            pack();
         }
      }
   ...
   public bool action(Event evt,Object arg)
   {
      ...
      if(evt.target == buttonOk)
      {
         try
         {
            ...
            parent.particle.init(width,height,cellSize);
            parent.particle.resize(parent.particle.preferredSize());
            parent.pack();
            dispose();
         }
