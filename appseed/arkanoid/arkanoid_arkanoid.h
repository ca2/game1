...
namespace arkanoid
{
   ...
   public:
      ...
      struct Ball
      {
         ...
         void update(float dt,_Arkanoid * parkanoid)
         {

            if(m_rect.left < 0) m_v.x = ballVelocity;
            else if(m_rect.right > parkanoid->windowWidth) m_v.x = -ballVelocity;

            if(m_rect.top < 0) m_v.y = ballVelocity;
            else if(m_rect.bottom > parkanoid->windowHeight) m_v.y = -ballVelocity;
         ...
         void draw(::draw2d::graphics * pgraphics)
         {
            ...
            pgraphics->StretchBlt(m_rect.left,m_rect.top,m_rect.width(),m_rect.height(),m_dib->get_graphics(),0,0,m_dib->m_size.cx,m_dib->m_size.cy,SRCCOPY);
      ...
      struct Paddle: public Rectangle
      {
         ...
         void key_left_up(_Arkanoid * parkanoid)
         {
            
            iLeft = 1;
         }

         void key_right_up(_Arkanoid * parkanoid)
         {
            iLeft = -1;

         }

         void key_down(_Arkanoid * parkanoid)
         {
      ...
      struct Brick: public Rectangle
      {
         ...
         void draw(::draw2d::graphics * pgraphics)
         {
            ...
            for(index i = 0; i < c; i++)
            {
               int a = ((c - i) * 123) / c;
               r.inflate(1,1);
               pgraphics->Draw3dRect(r,ARGB(a,49,184,255),ARGB(a,49,184,255));
            }

            Rectangle::draw(pgraphics);
      ...
      _Arkanoid(::aura::application * papp) :
      ...
      void draw(::draw2d::graphics * pdib)
      {
         ...
         bricks.pred_remove([](const Brick& mBrick){ return mBrick.destroyed; });


         ball.draw(pdib);
         paddle.draw(pdib);
         bricks.each([=](Brick& mBrick){ return mBrick.draw(pdib); });
